...
import { approveAIAction, executeAIAction, rejectAIAction } from '../ai-actions/aiActionsApi'
...
export function PersistentAIAssistant() {
  ...
  async function handleApprove(action: ProposedAgentAction, messageId: string, index: number) {
    ...
    try { await approveAIAction(action.proposal_id); setActionState(id, { status: 'approved' }) } catch (e) { setError(e instanceof Error ? e.message : 'Unable to approve action.'); setActionState(id, { status: 'failed' }) } finally { setActionBusy(null) }
  }
  ...
  async function handleReject(action: ProposedAgentAction, messageId: string, index: number) {
    ...
    const id = actionId(action, messageId, index)
    setActionBusy(id); setError(null)
    try { await rejectAIAction(action.proposal_id); setActionState(id, { status: 'rejected' }) } catch (e) { setError(e instanceof Error ? e.message : 'Unable to reject action.'); setActionState(id, { status: 'failed' }) } finally { setActionBusy(null) }
  }
  ...
  async function handleExecute(action: ProposedAgentAction, messageId: string, index: number) {
    ...
    const id = actionId(action, messageId, index)
    setActionBusy(id); setError(null)
    try { const execution = await executeAIAction(action.proposal_id); setActionState(id, { status: execution.success ? 'executed' : 'failed', result: execution.result }) } catch (e) { setError(e instanceof Error ? e.message : 'Unable to execute action.'); setActionState(id, { status: 'failed' }) } finally { setActionBusy(null) }
  }
  ...
  async function handleFeedback(action: ProposedAgentAction, messageId: string, index: number, rating: 'helpful' | 'not_helpful') {
    ...
    try {
      await recordAIFeedback({ action_id: action.proposal_id, recommendation_type: action.action_type, rating, outcome: state?.status === 'executed' ? 'successful' : state?.status === 'failed' ? 'unsuccessful' : 'unknown', note: null, context: { source: 'persistent_assistant' } })
  ...
  async function send(value = input) {
    ...
    try { const result = await sendPersistentMessage(conversation.id, content, conversation.context_horizon === 'week' ? 'week' : 'today'); setMessages(current => [...current.filter(m => m.id !== optimistic.id), result.userMessage, result.assistantMessage]); setConversations(await listConversations()) } catch (e) { setMessages(current => current.filter(m => m.id !== optimistic.id)); setError(e instanceof Error ? e.message : 'The assistant could not respond.') } finally { setLoading(false) }
  }
