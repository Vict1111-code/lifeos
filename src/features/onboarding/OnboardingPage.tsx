...
export function OnboardingPage() {
  ...
  const next = () => { setError(null); setStep((current) => Math.min(current + 1, STEPS.length - 1)); };
  const back = () => { setError(null); setStep((current) => Math.max(current - 1, 0)); };
  const toggleArea = (id: string) => setAreas((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]);
  const updateGoal = (index: number, patch: Partial<GoalDraft>) => setGoals((current) => current.map((goal, i) => i === index ? { ...goal, ...patch } : goal));
  const addGoal = () => { if (goals.length < 5) setGoals((current) => [...current, { title: '', life_area_category: areas[0] ?? 'career', why: '' }]); };

  async function finish(event?: FormEvent) {
    event?.preventDefault();
    if (!profile) return;
    setError(null); setSaving(true);
    const cleanGoals = goals.filter((goal) => goal.title.trim()).map((goal, index) => ({ ...goal, priority: index === 0 ? 'high' : 'medium' }));
    ...
    const { error: rpcError } = await supabase.rpc('complete_onboarding', {
      p_first_name: firstName, p_last_name: lastName, p_display_name: displayName,
      p_timezone: timezone, p_locale: 'en-NG', p_ai_enabled: aiEnabled,
      p_autonomy_level: Number(autonomy), p_life_areas: lifeAreas, p_goals: cleanGoals,
    });
    if (rpcError) { setSaving(false); setError(rpcError.message); return; }
    try { await refreshProfile(); } catch (refreshError) { console.error('Failed to refresh LifeOS profile:', refreshError); }
    setSaving(false); navigate('/', { replace: true });
  }

  return (
    <main className="min-h-screen bg-background px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-5xl flex-col">
        <header className="flex items-center justify-between gap-4 border-b pb-5"><div><p className="text-sm font-semibold text-primary">LifeOS</p><p className="text-xs text-muted-foreground">Set up your personal operating system</p></div><span className="text-xs font-medium text-muted-foreground">Step {step + 1} of {STEPS.length}</span></header>
        <div className="mt-6 grid gap-8 lg:grid-cols-[220px_1fr]">
          <nav className="hidden lg:block" aria-label="Onboarding progress"><ol className="space-y-2">{STEPS.map((label, index) => <li key={label} className={`rounded-xl px-3 py-2 text-sm ${index === step ? 'bg-primary/10 font-semibold text-primary' : index < step ? 'text-foreground' : 'text-muted-foreground'}`}><span className="mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full border text-xs">{index < step ? <Check className="h-3.5 w-3.5" /> : index + 1}</span>{label}</li>)}</ol></nav>
          <section className="flex min-h-[620px] flex-col rounded-3xl border bg-card p-6 shadow-sm sm:p-10">
            {step === 0 && <div className="m-auto max-w-2xl text-center"><div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Sparkles className="h-8 w-8" /></div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-primary">Your life, intentionally designed</p><h1 className="mt-4 text-4xl font-bold tracking-tight sm:text-5xl">Welcome to LifeOS.</h1><p className="mx-auto mt-5 max-w-xl text-base leading-7 text-muted-foreground">We’ll spend a few minutes establishing your direction, priorities, routines and AI preferences. You can change everything later.</p><button onClick={next} className="mt-8 inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground">Build my LifeOS <ArrowRight className="h-4 w-4" /></button></div>}
            {step === 1 && <div className="mx-auto w-full max-w-xl"><p className="text-sm font-semibold text-primary">Profile</p><h2 className="mt-2 text-3xl font-bold tracking-tight">How should LifeOS address you?</h2><p className="mt-2 text-muted-foreground">Use the name that feels natural for your daily briefs and reflections.</p><div className="mt-8 grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium">First name<input value={firstName} onChange={(e) => setFirstName(e.target.value)} className="mt-2 w-full rounded-xl border bg-background px-4 py-3 outline-none focus:ring-2 focus:ring-ring" required /></label><label className="text-sm font-medium">Last name<input value={lastName} onChange={(e) => setLastName(e.target.value)} className="mt-2 w-full rounded-xl border bg-background px-4 py-3 outline-none focus:ring-2 focus:ring-ring" /></label></div><label className="mt-4 block text-sm font-medium">Display name<input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder={firstName || 'Your preferred name'} className="mt-2 w-full rounded-xl border bg-background px-4 py-3 outline-none focus:ring-2 focus:ring-ring" /></label></div>}
