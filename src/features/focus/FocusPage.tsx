...
export function FocusPage() {
  ...
  const [targets, setTargets] = useState<{ tasks: { id: string; title: string; goal_id: string | null }[]; goals: { id: string; title: string }[] }>({ tasks: [], goals: [] })
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (current) { setMinutes(current.planned_minutes); setSelectedTask(current.task_id ?? ''); setSelectedGoal(current.goal_id ?? ''); setElapsed(Math.max(0, Math.floor((Date.now() - new Date(current.started_at).getTime()) / 1000))) }
    } catch (e) { setError(e instanceof Error ? e.message : 'Unable to load focus sessions.') }
    finally { setLoading(false) }
  }, [])

  useEffect(() => { void load() }, [load])
  useEffect(() => { if (!active) return; const timer = window.setInterval(() => setElapsed(Math.max(0, Math.floor((Date.now() - new Date(active.started_at).getTime()) / 1000))), 1000); return () => window.clearInterval(timer) }, [active])
  ...
  async function start() {
    ...
    try { const session = await startFocusSession({ planned_minutes: minutes, task_id: selectedTask || null, goal_id: selectedGoal || selectedTaskData?.goal_id || null }); setActive(session); setElapsed(0); setSessions(current => [session, ...current]) }
  ...
  async function finish(cancelled = false) {
    if (!active) return
    setBusy(true); setError(null)
    try { const updated = cancelled ? await cancelFocusSession(active.id, elapsed / 60) : await completeFocusSession(active.id, elapsed / 60); setSessions(current => current.map(item => item.id === updated.id ? updated : item)); setActive(null); setElapsed(0) }
